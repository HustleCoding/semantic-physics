...
export class PhysicsWorld {
  ...
  private flameGradients: CanvasGradient[] = [];
  ...
  private zoneLabel(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, color: string, align: CanvasTextAlign = "left") {
    ctx.fillStyle = color;
  ...
  // Everything that never moves is rasterised once into an offscreen canvas.
  private staticBackground() {
    ...
    ctx.fillRect(w.x, w.y + 4, w.width, w.height - 4);
    this.zoneLabel(ctx, "water", w.x + 12, w.y + 22, "#2d5f92");
    ...
    ctx.closePath();
    ctx.fill();
    this.zoneLabel(ctx, "heat lamp", cx, h.height + 14, "#a15f2b", "center");
    ...
    ctx.textAlign = "left";
    this.zoneLabel(ctx, "magnet", m.x - 8, m.y + m.height / 2 + 4, "#4a453d", "right");
  ...
  private flameGradient(index: number, baseY: number) {
    if (!this.flameGradients[index]) {
      const grad = this.ctx.createLinearGradient(0, baseY, 0, baseY - 60);
      grad.addColorStop(0, "rgba(217,84,43,.95)");
      grad.addColorStop(0.6, "rgba(240,160,60,.85)");
      grad.addColorStop(1, "rgba(255,220,120,0)");
      this.flameGradients[index] = grad;
    }
    return this.flameGradients[index];
  }

  // Only the animated parts: water surface, flames, lamp glow, magnet field.
  private drawZones() {
    const { ctx } = this;
    const t = performance.now() / 1000;

    const w = ZONES.water;
    ctx.fillStyle = "rgba(61,123,184,.30)";
    ctx.beginPath();
    ctx.moveTo(w.x, w.y + 6);
    for (let x = w.x; x <= w.x + w.width; x += 6) {
      ctx.lineTo(x, w.y + Math.sin(x / 28 + t * 2.2) * 2.5 + Math.sin(x / 11 - t * 3) * 1.2);
    }
    ctx.lineTo(w.x + w.width, w.y + 6);
    ctx.closePath();
    ctx.fill();
    ...
    for (let i = 0; i < 14; i++) {
      ...
      const half = 7 + (i % 2) * 3;
      ctx.fillStyle = this.flameGradient(i, f.y + f.height - 8);
      ctx.beginPath();
    ...
    this.zoneLabel(ctx, "fire", f.x + f.width / 2, f.y + f.height - 14, "#f8f4ec", "center");
    ...
    ctx.beginPath();
    for (let i = 1; i <= 3; i++) {
      const r = m.height / 2 + i * 12 + Math.sin(t * 2 + i) * 1.5;
      ctx.moveTo(m.x + m.width / 2 + Math.cos(Math.PI * 0.6) * r, m.y + m.height / 2 + Math.sin(Math.PI * 0.6) * r);
      ctx.arc(m.x + m.width / 2, m.y + m.height / 2, r, Math.PI * 0.6, Math.PI * 1.4);
    }
    ctx.stroke();
  }
  ...
  private drawObject(object: PhysicsObject) {
    ...
    ctx.closePath();
    const glowing = object.state.burning || object.material === "energy";
    ...
    if (glowing) {
      ctx.strokeStyle = "rgba(217,84,43,.35)";
      ctx.lineWidth = 10;
      ctx.stroke();
    }
    ...
    ctx.restore();
    ctx.globalAlpha = 1;
    const radius = object.body.circleRadius ?? Math.max(...vertices.map((v) => Math.hypot(v.x - object.body.position.x, v.y - object.body.position.y)));
    ctx.fillStyle = "#1c1a17";
